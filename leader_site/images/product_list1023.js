        ...
        success: function (data) {
            ...
            if (returnData.length >= 1) {
                for (var i = 0; i < returnData.length; i++) {
                    var sfts = returnData[i].sfts;//是否退市
                    var picUrl = returnData[i].appfile;//产品图片
                    var docPubUrl = returnData[i].docpuburl;//产品链接
                    var rqcp = returnData[i].rqcp;//是否人气
                    var sssj = returnData[i].shangshishijian;//上市时间
                    var pName = returnData[i].pname;//产品名称
                    var modelNo = returnData[i].modelno;//产品型号
                    var price = returnData[i].price;//价格
                    var cxtxbq = returnData[i].cxtxbq;//产品特性标签
                    var hxmd = returnData[i].hxmd;//核心卖点
                    var cplx = returnData[i].cplx;//产品类型
                    var cpfl = returnData[i].cpfl;//产品分类
                    var curDate = new Date().getTime();//当前时间
                    var newsssj = new Date(sssj).getTime();//上市时间处理
                    var bdate = 180 * 24 * 3600 * 1000;
                    //if(sfts=="否"){//判断是否下市
                    if (picUrl == "" || picUrl == null) {//图片处理
                ...
                //-------------------分页
                $(".cui_pager").html("");
                if (returnData.count >= 24) {
                    var splitPage = splitPageHTML(sword, _tableName, _curPage, _pageSize, returnData.count, order);
                    jQuery(".cui_pager").append(splitPage);
                }

                $(".js_fenye").on("click", function () {
                    var gao = $(".js_proList").offset().top;
                    if (gao > 300) {
                        $("body").animate({scrollTop: gao});
                    }
                    var change = $(this).attr("change");
                    change = change.split("|");
                    server(change[0].replace(/\'/g, ""), change[1].replace(/\'/g, ""), change[2], change[3], change[4].replace(/\'/g, ""));//server(change[0].replace(/\'/g,""),change[1].replace(/\'/g,""),change[2].replace(/\'/g,""),change[3],change[4]);
                });
                //分页结束
            }
        }
    ...
    $("." + onclickClass).click(function () {
        ...
        if ($(this).hasClass("active")) {
            ...
            if (!isSelectFlag) {
                ...
                appendFilterItemShowBar($(this).text());
 ...
 * 拼接已筛选的筛选项
 */
function appendFilterItemShowBar(showName,inputName,query){
    var itemShowBar ="";
    itemShowBar+='<div class="list-select-bg" inputName="'+inputName+'" query="'+query+'">';
