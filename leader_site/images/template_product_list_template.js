	...
	$('.js_recomChange').click(function(){
		$.ajax({
	        ...
	        success_cb: function(data){
	           	...
	           	var recommendData="";
	           	for(var j=0; j<4; j++){
	           		var pname=data[Math.round(Math.random()*data.length-1)].pname;
                    var modelno=data[Math.round(Math.random()*data.length-1)].modelno;
                    var dochref=data[Math.round(Math.random()*data.length-1)].dochref;
                    var pic=data[Math.round(Math.random()*data.length-1)].pic;
                    var sku_value=data[Math.round(Math.random()*data.length-1)].sku_value;
                    var minPrice = 0;
